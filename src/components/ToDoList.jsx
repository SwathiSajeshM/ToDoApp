...
const ToDoList = () => {
  ...
  const addTodo = () => {
    ...
    if(editId){
      ...
      const updateTodo = todos.map((swa)=>swa.id===editTodo.id
      ?(swa ={id : swa.id , list : todo})
      :(swa ={id : swa.id , list : swa.list}))
      setTodos(updateTodo)
      setEditId(0);
      setTodo('')
    }
        ...
        <ul className="list">
          {todos.map((swa) => (
            <li key={swa.id} className="list-items">
              <div
                className={`list-item-list ${swa.status ? "completed" : ""}`}
              >
                {swa.list}
              </div>
              <span>
